...
static void init(common_params &   params,
    ...
    if (params.n_parallel < 0) {
        LOG_INF(
            "%s: n_parallel is set to auto, using n_parallel = 4 and "
            "kv_unified = true\n",
            __func__);
        params.n_parallel = 4;
    ...
    if (params.ui_mcp_proxy || params.webui_mcp_proxy) {
        SRV_WRN("%s", "-----------------\n");
        SRV_WRN("%s",
                "CORS proxy is enabled, do not expose server to untrusted "
                "environments\n");
        SRV_WRN("%s",
                "This feature is EXPERIMENTAL and may be removed or changed "
                "in future versions\n");
        SRV_WRN("%s", "-----------------\n");
    ...
    if (is_router_server) {
        ...
        LOG_INF(
            "%s:       it is not recommended to use this mode in untrusted "
            "environments\n",
            __func__);
        if (ctx_http.thread.joinable()) {
