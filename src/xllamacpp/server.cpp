...
struct server_slot {
  ...
  void prompt_load(server_prompt_cache &prompt_cache,
...
struct server_context {
  ...
  bool load_model(const common_params &params) {
    ...
    if (!mmproj_path.empty()) {
      ...
      mparams.verbosity = params_base.verbosity > 0 ? GGML_LOG_LEVEL_DEBUG
                                                    : GGML_LOG_LEVEL_INFO;
  ...
  void receive_multi_results(
    ...
    for (int i = 0; i < (int)id_tasks.size(); i++) {
      server_task_result_ptr result =
          queue_results.recv_with_timeout(id_tasks, HTTP_POLLING_SECONDS);
...
static void handle_completions_impl(server_context &ctx_server,
  ...
  try {
    ...
    const size_t n_ctx_slot =
        ctx_server.n_ctx / ctx_server.params_base.n_parallel;
    ...
    res_error(format_error_response(e.what(), ERROR_TYPE_INVALID_REQUEST));
    return;
  }
