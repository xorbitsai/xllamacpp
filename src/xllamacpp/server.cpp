...
struct slot_params {
  ...
  int32_t n_indent = 0;   // minimum line indentation for the generated text in
  ...
  // Embeddings
  int32_t embd_normalize = 2; // (-1=none, 0=max absolute int16, 1=taxicab,
                              // 2=Euclidean/L2, >2=p-norm)
  ...
  json to_json(bool only_metrics = false) const {
    ...
    if (only_metrics) {
      return json{
          ...
          {"chat_format",
           common_chat_format_name(oaicompat_chat_syntax.format)},
          {"reasoning_format", common_reasoning_format_name(
    ...
    return json{
        ...
        {"reasoning_format",
         common_reasoning_format_name(oaicompat_chat_syntax.reasoning_format)},
        {"reasoning_in_content", oaicompat_chat_syntax.reasoning_in_content},
        {"thinking_forced_open", oaicompat_chat_syntax.thinking_forced_open},
        {"samplers", samplers},
        {"speculative.n_max", speculative.n_max},
...
struct server_task {
  ...
  static slot_params params_from_json_cmpl(const llama_context *ctx,
    ...
    params.sampling.top_n_sigma =
        json_value(data, "top_n_sigma", defaults.sampling.top_n_sigma);
    ...
    if (data.contains("logprobs") &&
        params.sampling.n_probs == defaults.sampling.n_probs) {
      params.sampling.n_probs =
          json_value(data, "logprobs", defaults.sampling.n_probs);
    }

    if (data.contains("lora")) {
      if (data.at("lora").is_array()) {
        params.lora =
            parse_lora_request(params_base.lora_adapters, data.at("lora"));
      } else {
        throw std::runtime_error("Error: 'lora' must be an array of objects "
                                 "with 'id' and 'scale' fields");
      }
    } else {
    ...
    if (data.contains("json_schema") && !data.contains("grammar")) {
      ...
      params.sampling.grammar_lazy =
          json_value(data, "grammar_lazy", defaults.sampling.grammar_lazy);
...

    {
      ...
      if (it != data.end()) {
        params.oaicompat_chat_syntax.format =
            static_cast<common_chat_format>(it->get<int>());
        SRV_INF("Chat format: %s\n",
                common_chat_format_name(params.oaicompat_chat_syntax.format));
      } else {
        params.oaicompat_chat_syntax.format =
            defaults.oaicompat_chat_syntax.format;
      }
      common_reasoning_format reasoning_format = params_base.reasoning_format;
      if (data.contains("reasoning_format")) {
        reasoning_format = common_reasoning_format_from_name(
            data.at("reasoning_format").get<std::string>());
      }
      ...
      params.oaicompat_chat_syntax.thinking_forced_open =
          json_value(data, "thinking_forced_open", false);
      params.oaicompat_chat_syntax.parse_tool_calls =
          json_value(data, "parse_tool_calls", false);
    }

    {
      ...
      if (preserved_tokens != data.end()) {
        for (const auto &t : *preserved_tokens) {
          auto ids = common_tokenize(vocab, t.get<std::string>(),
                                     /* add_special= */ false,
                                     /* parse_special= */ true);
          if (ids.size() == 1) {
          ...
          if (ct.value.type == COMMON_GRAMMAR_TRIGGER_TYPE_WORD) {
            ...
            if (ids.size() == 1) {
              ...
              if (std::find(params.sampling.preserved_tokens.begin(),
                            ...
                            (llama_token)token) ==
                  params.sampling.preserved_tokens.end()) {
                throw std::runtime_error("Grammar trigger word should be "
                                         "marked as preserved token: " +
                                         word);
              }
        ...
        for (const auto &el : *logit_bias) {
          ...
          if (el.is_array() && el.size() == 2) {
            ...
            if (el[0].is_number_integer()) {
              ...
              auto toks =
                  common_tokenize(vocab, el[0].get<std::string>(), false);
              for (auto tok : toks) {
                params.sampling.logit_bias.push_back({tok, bias});
              }
            }
      ...
      if (samplers != data.end()) {
        if (samplers->is_array()) {
          params.sampling.samplers =
              common_sampler_types_from_names(*samplers, false);
        } else if (samplers->is_string()) {
          params.sampling.samplers =
              common_sampler_types_from_chars(samplers->get<std::string>());
        }
      } else {
          ...
          {"choices",
           json::array({
               json{
                   ...
                   {"index", 0},
                   {"delta",
                    common_chat_msg_diff_to_json_oaicompat<json>(diff)},
               },
           })},
          {"created", t},
          {"id", oaicompat_cmpl_id},
...
struct server_prompt_cache {
  server_prompt_cache(int32_t limit_size_mib, size_t limit_tokens) {
    this->limit_size =
        1024ull * 1024ull * (limit_size_mib < 0 ? 0 : limit_size_mib);
  ...
  server_prompt *alloc(const server_prompt &prompt, size_t state_size) {
    ...
    // next, remove any cached prompts that are fully contained in the current
    // prompt
    ...
    // TODO: for some reason we can't copy server_tokens, so we have to do this
    // workaround
  ...
  bool load(server_prompt &prompt, const server_tokens &tokens_new,
    ...
    // find the most similar cached prompt, that would also preserve the most
    // context
    for (auto it = states.begin(); it != states.end(); ++it) {
    ...
    if (it_best != states.end()) {
      ...
      const size_t n = llama_state_seq_set_data_ext(ctx, it_best->data.data(),
                                                    size, id_slot, 0);
      if (n != size) {
  ...
  void update() {
    if (limit_size > 0) {
      ...
      while (states.size() > 1 && size() > limit_size) {
        ...
        SRV_WRN(" - cache size limit reached, removing oldest entry (size = "
                "%.3f MiB)\n",
    ...
    const float size_per_token = std::max<float>(
        1.0f, float(size()) / (std::max<size_t>(1, n_tokens())));
    ...
    if (limit_tokens > 0) {
      while (states.size() > 1 && n_tokens() > limit_tokens_cur) {
        ...
        SRV_WRN(" - cache token limit (%zu, est: %zu) reached, removing oldest "
                "entry (size = %.3f MiB)\n",
    ...
    SRV_WRN(" - cache state: %zu prompts, %.3f MiB (limits: %.3f MiB, %zu "
            "tokens, %zu est)\n",
...
struct server_slot {
  ...
  bool prompt_load(server_prompt_cache &prompt_cache,
  ...
  update_chat_msg(std::vector<common_chat_msg_diff> &diffs) {
    ...
    if (!new_msg.empty()) {
      ...
      chat_msg = new_msg;
      diffs = common_chat_msg_diff::compute_diffs(
...
private:
  void cleanup_pending_task(int id_target) {
    ...
    queue_tasks.erase(
        ...
        queue_tasks.end());
    queue_tasks_deferred.erase(std::remove_if(queue_tasks_deferred.begin(),
...
struct server_response {
  ...
  recv_with_timeout(const std::unordered_set<int> &id_tasks, int timeout) {
    while (true) {
      std::unique_lock<std::mutex> lock(mutex_results);
...
struct server_context {
  ...
  // note: keep these alive - they determine the lifetime of the model, context,
  // etc.
  ...
  bool load_model(const common_params &params) {
    ...
    if (params_base.has_speculative()) {
      ...
      params_dft.cpuparams.n_threads =
          params_base.speculative.cpuparams.n_threads;
      params_dft.cpuparams_batch.n_threads =
          params_base.speculative.cpuparams_batch.n_threads;
      params_dft.tensor_buft_overrides =
          params_base.speculative.tensor_buft_overrides;
      ...
      vocab_dft_compatible =
          common_speculative_are_compatible(ctx, llama_init_dft.context.get());
      if (!vocab_dft_compatible) {
        SRV_INF("the draft model '%s' is not compatible with the target model "
                "'%s'. tokens will be translated between the draft and target "
                "models.\n",
                params_base.speculative.model.path.c_str(),
                params_base.model.path.c_str());
      }
    ...
    chat_templates =
        common_chat_templates_init(model, params_base.chat_template);
    try {
      common_chat_format_example(chat_templates.get(), params.use_jinja,
                                 params.default_template_kwargs);
    } catch (const std::exception &e) {
      SRV_WRN("%s: Chat template parsing error: %s\n", __func__, e.what());
      SRV_WRN("%s: The chat template that comes with this model is not yet "
  ...
  void init() {
    ...
    if (n_ctx_slot > n_ctx_train) {
      SRV_WRN("the slot context (%d) exceeds the training context of the model "
              "(%d) - capping\n",
      ...
      if (model_dft) {
        ...
        for (auto &pair : params_base.speculative.replacements) {
          common_speculative_add_replacement_tgt_dft(
              slot.spec, pair.first.c_str(), pair.second.c_str());
        }
      }
...

    {
      ...
      slots_debug =
          LLAMA_SERVER_SLOTS_DEBUG ? atoi(LLAMA_SERVER_SLOTS_DEBUG) : 0;
    ...
    SRV_WRN(
        "%s",
        "for more info see https://github.com/ggml-org/llama.cpp/pull/16391\n");
  ...
  server_slot *get_available_slot(const server_task &task) {
    ...
    if (ret == nullptr && slot_prompt_similarity != 0.0f) {
      ...
      for (server_slot &slot : slots) {
        ...
        // fraction of the Longest Common Prefix length with respect to the
        // input prompt length
        const float sim_cur =
            float(tokens.get_common_prefix(task.tokens)) / task.tokens.size();
      ...
      if (ret != nullptr) {
        const float f_keep =
            (sim_best * task.tokens.size()) / ret->prompt.tokens.size();

        SLT_INF(*ret,
                "selected slot by LCP similarity, sim_best = %.3f (> %.3f "
                "thold), f_keep = %.3f\n",
                sim_best, slot_prompt_similarity, f_keep);

        // if we are about to lose a large portion of the existing context -
        // save it in the prompt cache
        if (f_keep < 0.5f) {
          update_cache = true;
        }
      }
  ...
  bool launch_slot_with_task(server_slot &slot, server_task &&task) {
    ...
    if (lora_all_alora(slot.lora)) {
      ...
      if (enabled_ids.size() != 1) {
        send_error(task, "Cannot run multiple aLoRAs in a single request",
                   ERROR_TYPE_INVALID_REQUEST);
        return false;
      }
      ...
      const uint64_t n_invocation_tokens =
          llama_adapter_get_alora_n_invocation_tokens(lora);
      const llama_token *invocation_tokens =
          llama_adapter_get_alora_invocation_tokens(lora);
    ...
    if (!task.tokens.validate(ctx)) {
      send_error(task, "Prompt contains invalid tokens",
                 ERROR_TYPE_INVALID_REQUEST);
      return false;
    }
    ...
    if (slot.ctx_dft) {
      ...
      slot.batch_spec =
          llama_batch_init(task.params.speculative.n_max + 1, 0, 1);
    }
  ...
  bool process_token(completion_token_output &result, server_slot &slot) {
    ...
    if (!params_base.ctx_shift && slot.prompt.n_tokens() + 1 >= slot.n_ctx) {
      ...
      SLT_DBG(
          slot,
          "stopped due to running out of context capacity, prompt.n_tokens() = "
          "%d, task.n_tokens = %d, n_decoded = %d, n_ctx = %d\n",
          slot.prompt.n_tokens(), slot.task->n_tokens(), slot.n_decoded,
          slot.n_ctx);
    }
    ...
    if (result.text_to_send.find('\n') != std::string::npos) {
      ...
      // if we have seen a new line, we stop after a certain time limit, but
      // only upon another new line
  ...
  void send_error(const int id_task, const std::string &error,
                  const enum error_type type = ERROR_TYPE_SERVER,
                  const int32_t n_prompt_tokens = 0, const int32_t n_ctx = 0) {
  ...
  bool check_no_mtmd(const int id_task) {
    if (mctx) {
      send_error(id_task, "This feature is not supported by multimodal",
  ...
  void process_single_task(server_task &&task) {
    ...
    case SERVER_TASK_TYPE_SLOT_RESTORE: {
      ...
      size_t nread =
          llama_state_seq_load_file(ctx, filepath.c_str(), slot->id,
  ...
  void update_slots() {
    ...
    for (server_slot &slot : slots) {
      if (slot.state == SLOT_STATE_GENERATING &&
        ...
        if (mctx) {
          // we should never reach this because params_base.ctx_shift is
          // automatically disabled if mmproj is loaded we don't support
          // ctx_shift because an image chunk may contains multiple tokens
        ...
        const int n_discard = slot.task->params.n_discard
        ...
        llama_memory_seq_add(llama_get_memory(ctx), slot.id, n_keep + n_discard,
                             slot.prompt.n_tokens(), -n_discard);

        // add generated tokens to cache
        // ref:
        // https://github.com/ggml-org/llama.cpp/pull/16818#discussion_r2473269481
        {
          GGML_ASSERT(!slot.prompt.tokens.has_mtmd);

          llama_tokens new_tokens =
              slot.prompt.tokens.get_text_tokens(); // copy
    ...
    for (auto &slot : slots) {
      ...
      common_batch_add(batch, slot.sampled, slot.prompt.tokens.pos_next(),
                       {slot.id}, true);

      slot.prompt.tokens.push_back(slot.sampled);

      SLT_DBG(slot,
              "slot decode token, n_ctx = %d, n_tokens = %d, truncated = %d\n",
      ...
      for (auto &slot : slots) {
        ...
        if (slot.state == SLOT_STATE_PROCESSING_PROMPT ||
          ...
          if (slot.state == SLOT_STATE_STARTED) {
            ...
            SLT_INF(slot,
                    "new prompt, n_ctx_slot = %d, n_keep = %d, task.n_tokens = "
                    "%d\n",
                    slot.n_ctx, slot.task->params.n_keep,
                    slot.task->n_tokens());
            ...
            common_token_to_piece(ctx, input_tokens[i]).c_str());
                }
            } else {
                // all
                for (int i = 0; i < (int) input_tokens.size(); i++) {
                    SLT_DBG(slot, "prompt token %3d: %6d '%s'\n", i,
            input_tokens[i], common_token_to_piece(ctx,
            input_tokens[i]).c_str());
                }
            }*/
            ...
            if (!slot.can_split()) {
              ...
              if (slot.task->params.cache_prompt) {
                ...
                // if there is an alora invoked, don't cache after the
                // invocation start
                if (slot.alora_invocation_start > 0) {
                  SLT_DBG(slot,
                          "only caching to alora invocation start (n_past = "
                ...
                if (params_base.n_cache_reuse > 0) {
                  ...
                  SLT_DBG(
                      slot,
                      "trying to reuse chunks with size > %d, n_past = %d\n",
                  ...
                  while (head_c < slot.prompt.tokens.size() &&
                    ...
                    while (head_c + n_match < slot.prompt.tokens.size() &&
                           ...
                           slot.prompt.tokens[head_c + n_match] ==
                               input_tokens[head_p + n_match]) {
                    ...
                    if (n_match >= (size_t)params_base.n_cache_reuse) {
                      ...
                      for (size_t i = 0; i < n_match; i++) {
                        slot.prompt.tokens.set_token(
                            head_p + i, slot.prompt.tokens[head_c + i]);
                        n_past++;
                      }
                ...
                // if we don't cache the prompt, we have to remove all previous
                // tokens
                n_past = 0;
              }

              // note: when n_swa == 0, the model does not use SWA, which is
              // equivalent to a window of 1
              ...
              if (!mctx && n_past > 0 && n_past < slot.prompt.n_tokens()) {
                ...
                if (pos_min == -1) {
                  SLT_ERR(slot,
                          "n_past = %d, slot.prompt.tokens.size() = %d, seq_id "
                          "= %d, pos_min = %d\n",
                  ...
                  GGML_ABORT(
                      ...
                      "13833#discussion_r2116181237");
                }

                // when the prompt prefix does not match, print the tokens
                // around the mismatch this is useful for debugging prompt
                // caching
                if (slots_debug) {
                  const int np0 = std::max<int>(n_past - 4, 0);
                  const int np1 = std::min<int>(
                      n_past + 6, std::min(slot.prompt.tokens.size(),
                                           slot.task->tokens.size()));
                ...
                if (pos_min > pos_min_thold) {
                  // TODO: support can be added in the future when corresponding
                  // vision models get released
                  GGML_ASSERT(!slot.prompt.tokens.has_mtmd);
                  ...
                  const auto it = std::find_if(
                      ...
                      slot.prompt.checkpoints.rend(), [&](const auto &cur) {
                        // guarantee that a checkpoint will result in at least
                        // one token being processed [TAG_PROMPT_LOGITS]
                  ...
                  if (!do_reset) {
                    ...
                    const size_t n = llama_state_seq_set_data_ext(
                        ctx, it->data.data(), checkpoint_size, slot.id,
                        LLAMA_STATE_SEQ_FLAGS_PARTIAL_ONLY);

                    if (n != checkpoint_size) {
                      SLT_ERR(slot,
                              "failed to restore context checkpoint (pos_min = "
                              "%d, pos_max = %d, size = %.3f MiB)\n",
                              it->pos_min, it->pos_max,
                              (float)checkpoint_size / 1024 / 1024);
                      do_reset = true;
                      // printf("[DEBUG] `do_reset` was set to `true` after
                      // failing to restore a checkpoint");
                    } else {
                      n_past = std::min(n_past,
                                        std::max(it->pos_min + 1, it->pos_max));
                      SLT_WRN(slot,
                              "restored context checkpoint (pos_min = %d, "
                              "pos_max = %d, size = %.3f MiB)\n",
                              it->pos_min, it->pos_max,
                              (float)checkpoint_size / 1024 / 1024);
                    }
                  }
                  ...
                  if (do_reset) {
                    SLT_WRN(slot,
                            ...
                            "cache data (likely due to SWA or hybrid/recurrent "
                            "memory, see %s)\n",
                            "https://github.com/ggml-org/llama.cpp/pull/"
...

              {
                ...
                for (auto it = slot.prompt.checkpoints.begin();
                  ...
                  if (cur.pos_min > pos_min_thold) {
                    SLT_WRN(slot,
                            "erased invalidated context checkpoint (pos_min = "
                            "%d, pos_max = %d, n_swa = %d, size = %.3f MiB)\n",
                            cur.pos_min, cur.pos_max, n_swa,
          ...
          if (!llama_memory_seq_rm(llama_get_memory(ctx), slot.id, p0, -1)) {
            SLT_WRN(slot,
                    "failed to truncate tokens with position >= %d - clearing "
                    "the memory\n",
                    p0);
          ...
          if (slot.prompt.n_tokens() < slot.task->n_tokens() &&
            ...
            {
              const auto &chunk =
                  input_tokens.find_chunk(slot.prompt.n_tokens());
          ...
          if (lora_all_alora(slot.lora) &&
              slot.alora_invocation_start - 1 > slot.prompt.n_tokens()) {
            SLT_DBG(slot,
                    "processing pre-alora tokens without the adapter (n_tokens "
                    "= %d, alora_invocation_start = %d)\n",
          ...
          do_checkpoint =
              do_checkpoint && slot.task->type == SERVER_TASK_TYPE_COMPLETION;

          // make a checkpoint of the parts of the memory that cannot be rolled
          // back. checkpoints are created only if:
          // - the model uses SWA and we are not using `swa_full`
          // - the model architecture is marked as recurrent or hybrid
          //
          // TODO: try to make this conditional on the context or the memory
          // module, instead of the model type
          do_checkpoint =
              do_checkpoint &&
          ...
          while (slot.prompt.n_tokens() < slot.task->n_tokens() &&
            ...
            if (alora_scale > 0 &&
                slot.prompt.n_tokens() == slot.alora_invocation_start - 1) {
              SLT_DBG(slot,
                      "stop prompt batch filling at (n_tokens = %d, "
            ...
            common_batch_add(batch, cur_tok, slot.prompt.tokens.pos_next(),
                             {slot.id}, slot.need_embd());
            slot.prompt.tokens.push_back(cur_tok);

            slot.n_prompt_tokens_processed++;

            // process the last few tokens of the prompt separately in order to
            // allow for a checkpoint to be created.
            if (do_checkpoint &&
                slot.task->n_tokens() - slot.prompt.n_tokens() == 64) {
              break;
            }
          }
          ...
          SLT_INF(slot,
                  "prompt processing progress, n_tokens = %d, batch.n_tokens = "
                  "%d, progress = %f\n",
          ...
          if (slot.prompt.n_tokens() == slot.task->n_tokens()) {
            ...
            const auto pos_min =
                llama_memory_seq_pos_min(llama_get_memory(ctx), slot.id);
            const auto pos_max =
                llama_memory_seq_pos_max(llama_get_memory(ctx), slot.id);
            ...
            if (do_checkpoint) {
              while (slot.prompt.checkpoints.size() >=
                     (size_t)params_base.n_ctx_checkpoints) {
                // make room for the new checkpoint, if needed
                const auto &cur = slot.prompt.checkpoints.front();

                SLT_WRN(slot,
                        "erasing old context checkpoint (pos_min = %d, pos_max "
                        "= %d, size = %.3f MiB)\n",
              ...
              const size_t checkpoint_size = llama_state_seq_get_size_ext(
              ...
              llama_state_seq_get_data_ext(ctx, cur.data.data(),
                                           checkpoint_size, slot.id,
                                           LLAMA_STATE_SEQ_FLAGS_PARTIAL_ONLY);

              SLT_WRN(slot,
                      "created context checkpoint %d of %d (pos_min = %d, "
                      "pos_max = %d, size = %.3f MiB)\n",
                      (int)slot.prompt.checkpoints.size(),
                      params_base.n_ctx_checkpoints, cur.pos_min, cur.pos_max,
                      (float)cur.data.size() / 1024 / 1024);
            }
          }
        }
    ...
    for (int32_t i = 0; i < batch.n_tokens; i = i_next) {
      ...
      if (ret != 0) {
        {
          ...
          if (n_batch == 1 && ret == 1) {
            // TODO: try to terminate only the largest active slot/sequence and
          ...
          if (ret < -1) {
            // TODO: update slot state based on llama_memory_seq_pos_min() and
          ...
          if (!err.empty()) {
            ...
            for (auto &slot : slots) {
              if (slot.is_processing()) {
                ...
                // note: it's complicated to keep track of how much of the
                // current batch has been
                //       processed before the error occurred, so we simply clear
                //       the entire context
                clear_slot(slot);
              }
            }
        ...
        SRV_WRN("failed to find free space in the KV cache, retrying with "
                "smaller batch size, i = %d, n_batch = %d, ret = %d\n",
                i, n_batch, ret);

        continue; // continue loop of n_batch
      }

      // move the head of the batch forward with the number of tokens we just
      // processed
      i_next = i + n_tokens;
      ...
      for (auto &slot : slots) {
        ...
        result.text_to_send = common_token_to_piece(
            ctx, result.tok, accept_special_token(slot, result.tok));
      ...
      // TODO: rework to have a single draft llama_context shared across all
      // slots [TAG_SERVER_SPEC_REWORK]
      //       perform the speculative drafting for all sequences at the same
      ...
      for (auto &slot : slots) {
        ...
        if (mctx) {
          // we should never reach this, as speculative is automatically
          // disabled if mmproj is loaded
          GGML_ABORT("not supported by multimodal");
        }
        ...
        // note: slot.prompt is not yet expanded with the `id` token sampled
        // above
        //       also, need to leave space for 1 extra token to allow context
        //       shifts
        n_draft_max =
            std::min(n_draft_max, slot.n_ctx - slot.prompt.n_tokens() - 2);
        ...
        params_spec.n_reuse =
            llama_n_ctx(slot.ctx_dft) - slot.task->params.speculative.n_max;
        ...
        common_batch_add(slot.batch_spec, id, slot.prompt.tokens.pos_next(),
                         {slot.id}, true);

        for (size_t i = 0; i < draft.size(); ++i) {
          common_batch_add(slot.batch_spec, draft[i],
                           slot.prompt.tokens.pos_next() + 1 + i, {slot.id},
                           true);
        }
...
// generator-like API for server responses, support pooling connection state and
// aggregating results
struct server_response_reader {
  ...
  bool has_next() { return !cancelled && received_count < id_tasks.size(); }

  // return nullptr if should_stop() is true before receiving a result
  // note: if one error is received, it will stop further processing and return
  // error result
  server_task_result_ptr next(const std::function<bool()> &should_stop) {
    while (true) {
      server_task_result_ptr result =
          ctx_server.queue_results.recv_with_timeout(id_tasks,
  ...
  struct batch_response {
    bool is_terminated = false; // if true, indicates that processing was
                                // stopped before all results were received
  ...
  batch_response wait_for_all(const std::function<bool()> &should_stop) {
    ...
    while (has_next()) {
      ...
      GGML_ASSERT(batch_res.results[idx] == nullptr &&
...
static void init(common_params &params, server_context &ctx_server,
  ...
  LOG_INF("%s: chat template, chat_template: %s, example_format: '%s'\n",
          ...
          common_chat_format_example(
              ctx_server.chat_templates.get(), ctx_server.params_base.use_jinja,
              ctx_server.params_base.default_template_kwargs)
              .c_str());
  ...
  ctx_server.queue_tasks.on_update_slots(
      [&ctx_server]() { ctx_server.update_slots(); });
...
handle_metrics_impl(server_context &ctx_server,
  ...
  if (!ctx_server.params_base.endpoint_metrics) {
    res_err(format_error_response("This server does not support metrics "
      ...
      json{{"counter",
            {{{"name", "prompt_tokens_total"},
             ...
             {{"name", "n_busy_slots_per_decode"},
              ...
              {"value", (float)res_task->n_busy_slots_total /
                            std::max((float)res_task->n_decode_total, 1.f)}}}},
           {"gauge",
            {{{"name", "prompt_tokens_seconds"},
              {"help", "Average prompt throughput in tokens/s."},
              {"value", res_task->n_prompt_tokens_processed
                            ? 1.e3 / res_task->t_prompt_processing *
                                  res_task->n_prompt_tokens_processed
                            : 0.}},
             {{"name", "predicted_tokens_seconds"},
              {"help", "Average generation throughput in tokens/s."},
              {"value", res_task->n_tokens_predicted
                            ? 1.e3 / res_task->t_tokens_generation *
                                  res_task->n_tokens_predicted
                            : 0.}},
...
static void handle_completions_impl(server_context &ctx_server,
  ...
  // need to store the reader as a pointer, so that it won't be destroyed when
  // the handle returns use shared_ptr as it's shared between the
  // chunked_content_provider() and on_complete()
  ...
  try {
    ...
    // TODO: this log can become very long, put it behind a flag or think about
    ...
    if (oaicompat && ctx_server.mctx != nullptr) {
      // This is the case used by OAI compatible chat path with MTMD. TODO It
      // can be moved to the path below.
      inputs.push_back(process_mtmd_prompt(ctx_server.mctx,
                                           prompt.get<std::string>(), files));
    } else {
      // Everything else, including multimodal completions.
      inputs = tokenize_input_prompts(ctx_server.vocab, ctx_server.mctx, prompt,
                                      true, true);
    }
    ...
    res_err(format_error_response(e.what(), ERROR_TYPE_INVALID_REQUEST));
    return;
  }
  ...
  if (!stream) {
    ...
    if (all_results.is_terminated) {
      ...
      for (auto &res : all_results.results) {
        GGML_ASSERT(dynamic_cast<server_task_result_cmpl_final *>(res.get()) !=
                    nullptr);
        arr.push_back(res->to_json());
      }
      // if single request, return single object instead of array
      res_ok(arr.size() == 1 ? arr[0] : arr);
    }
...
static void handle_embeddings_impl(server_context &ctx_server, const json &data,
  ...
  if (!ctx_server.params_base.embedding) {
    res_err(format_error_response("This server does not support embeddings. "
  ...
  if (oaicompat != OAICOMPAT_TYPE_NONE &&
    ...
    res_err(format_error_response("Pooling type 'none' is not OAI compatible. "
  ...
  if (body.count("input") != 0) {
    ...
    res_err(format_error_response("\"input\" or \"content\" must be provided",
                                  ERROR_TYPE_INVALID_REQUEST));
    return;
  }
  ...
  if (body.count("encoding_format") != 0) {
    ...
    if (format == "base64") {
      ...
      res_err(format_error_response("The format to return the embeddings in. "
  ...
  auto tokenized_prompts = tokenize_input_prompts(
      ctx_server.vocab, ctx_server.mctx, prompt, true, true);
  for (const auto &tokens : tokenized_prompts) {
    // this check is necessary for models that do not add BOS token to the input
    if (tokens.empty()) {
      res_err(format_error_response("Input content cannot be empty",
                                    ERROR_TYPE_INVALID_REQUEST));
      return;
    }
  }
...
static void handle_rerank_impl(server_context &ctx_server, const json &data,
  ...
  if (body.count("query") == 1) {
    query = body.at("query");
    if (!query.is_string()) {
      res_err(format_error_response("\"query\" must be a string",
                                    ERROR_TYPE_INVALID_REQUEST));
      return;
    }
  } else {
    res_err(format_error_response("\"query\" must be provided",
                                  ERROR_TYPE_INVALID_REQUEST));
    return;
  }
  ...
  // write JSON response
  json root =
      format_response_rerank(body, responses, is_tei_format, documents, top_n);
  res_ok(root);
};
...
std::string Server::handle_rerank(const std::string &input_json_str) {
  ...
  std::string result;
  auto cb = [&result](const json &data) { result = data.dump(); };
  handle_rerank_impl(*_ctx_server, body, cb, cb, OAICOMPAT_TYPE_NONE);
  return result;
}
