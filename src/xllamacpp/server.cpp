    ...
    if (!msg.tool_calls.empty()) {
      ...
      for (const auto &tc : msg.tool_calls) {
        tool_calls.push_back({
            ...
            // Some templates generate and require an id (sometimes in a very
            // specific format, e.g. Mistral Nemo). We only generate a random id
            // for the ones that don't generate one by themselves (they also
            // won't get to see it as their template likely doesn't use it, so
...
static void handle_completions_impl(server_context &ctx_server,
  ...
  try {
    ...
    const bool has_mtmd = ctx_server.mctx != nullptr;
    {
      ...
      for (auto &file : files) {
        mtmd::bitmap bmp(
            mtmd_helper_bitmap_init_from_buf(file.data(), file.size()));
        if (!bmp.ptr) {
...
std::string get_json_device_info() {
  ...
  json json_device_info = json::array();
  ...
  for (size_t i = 0; i < dev_count; ++i) {
    ...
    printf("  Device memory: %zu MB (%zu MB free)\n", total / 1024 / 1024,
           free / 1024 / 1024);
    printf("\n");
    json_device_info.push_back(
        {{"name", ggml_backend_dev_name(devs[i])},
         {"description", ggml_backend_dev_description(devs[i])},
         {"free", free},
         {"total", total}});
  }
