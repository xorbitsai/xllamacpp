...
build_llamacpp() {
  ...
  if [[ -n "${XLLAMACPP_BUILD_CUDA}" ]]; then
    ...
    cmake --build . --config Release -j ${NPROC} --target common llama ggml ggml-blas ggml-cpu ggml-cuda mtmd_static
	...
	cmake --build . --config Release -j ${NPROC} --target common llama ggml ggml-blas ggml-cpu ggml-hip mtmd_static
  else
