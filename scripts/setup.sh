...
build_llamacpp() {
  ...
  if [[ -n "${XLLAMACPP_BUILD_CUDA}" ]]; then
	...
	cmake .. -DBUILD_SHARED_LIBS=OFF -DCMAKE_POSITION_INDEPENDENT_CODE=ON -DCMAKE_INSTALL_LIBDIR=lib -DLLAMA_CURL=OFF -DGGML_BACKEND_DL=ON -DGGML_NATIVE=OFF -DGGML_CPU=OFF -DGGML_CUDA=ON -DGGML_CUDA_FORCE_MMQ=ON && \
    cmake --build . --config Release -j ${NPROC} && \
    cmake --install . --prefix ${PREFIX}
  elif [[ -n "${XLLAMACPP_BUILD_HIP}" ]]; then
	echo "Building for AMD GPU"
	cmake .. -DBUILD_SHARED_LIBS=OFF -DCMAKE_POSITION_INDEPENDENT_CODE=ON -DCMAKE_INSTALL_LIBDIR=lib -DLLAMA_CURL=OFF -DGGML_BACKEND_DL=ON -DGGML_NATIVE=OFF -DGGML_CPU=OFF -DAMDGPU_TARGETS="gfx1100;gfx1101;gfx1102;gfx1030;gfx1031;gfx1032" -DCMAKE_HIP_COMPILER="$(hipconfig -l)/clang" -DGGML_HIP_ROCWMMA_FATTN=ON -DGGML_HIP=ON && \
	cmake --build . --config Release -j ${NPROC} && \
    cmake --install . --prefix ${PREFIX}
  else
