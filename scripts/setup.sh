...
build_llamacpp() {
  ...
  if [[ -n "${XLLAMACPP_BUILD_CUDA}" ]]; then
	cmake .. -DBUILD_SHARED_LIBS=OFF -DCMAKE_POSITION_INDEPENDENT_CODE=ON -DCMAKE_INSTALL_LIBDIR=lib -DGGML_CUDA=ON -DGGML_CUDA_FORCE_MMQ=ON && \
    cmake --build . --config Release -j ${NPROC} && \
    cmake --install . --prefix ${PREFIX}
  elif [[ -n "${XLLAMACPP_BUILD_HIP}" ]]; then
	cmake .. -DBUILD_SHARED_LIBS=OFF -DCMAKE_POSITION_INDEPENDENT_CODE=ON -DCMAKE_INSTALL_LIBDIR=lib -DCMAKE_HIP_COMPILER="$(hipconfig -l)/clang" -DGGML_HIP_ROCWMMA_FATTN=ON -DGGML_HIP=ON && \
	cmake --build . --config Release -j ${NPROC} && \
    cmake --install . --prefix ${PREFIX}
  else
